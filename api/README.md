...
## Mock-данные
В каталоге [`api/docs/fixtures`](./docs/fixtures/README.md) лежат готовые JSON-файлы для локальной разработки (пользователь, локации, одежда, погода, аутфиты) и пошаговая инструкция по их загрузке через `curl`/`jq`.
