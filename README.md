...
- Приложение экспонирует метрики Prometheus на `/metrics` и использует глобальный rate limiting (см. `settings.API_RATE_LIMIT`).
- В исходящих HTTP-вызовах реализованы тайм-ауты, rate limiting и circuit breaker (`services/http_client.py`), а трассировки публикуются в Jaeger при активированном `TRACING_ENABLED`.
- Поднимите полный стек мониторинга командой `docker compose up` из каталога `deploy/` — детальная инструкция и список портов приведены в `deploy/README.md`.
- Готовый Grafana-дэшборд и правила алертов расположены в `deploy/observability/`.
...
Для нагрузочных проверок используйте скрипты в `tests/load/` и рекомендации из `docs/performance/load_testing_report.md`.
