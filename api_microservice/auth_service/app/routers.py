"""FastAPI routers for user authentication and profile management."""
...
SECRET_KEY = settings.jwt_secret or "supersecretkey"
...
def _ensure_password_fits_backend(password: str) -> None:
    """Ensure the password length matches business rules and bcrypt limitations."""
    ...
    if len(password.encode("utf-8")) > _MAX_BCRYPT_BYTES:
        raise HTTPException(status_code=400, detail=_PASSWORD_TOO_LONG_DETAIL)
...
def register(user: schemas.UserCreate, db: Session = Depends(get_db)) -> schemas.UserResponse:
    ...
    new_user = models.User(
        ...
        style_preference=_normalize_style(user.style_preference),
        pin_hash=pin_hash,
    )
    db.add(new_user)
    db.commit()
...
def update_user(user_id: int, updates: schemas.UserUpdate, db: Session = Depends(get_db)) -> models.User:
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден")
    ...
    if updates.style_preference is not None:
        user.style_preference = _normalize_style(updates.style_preference)
...
def verify_pin(user_id: int, payload: schemas.PinVerificationRequest, db: Session = Depends(get_db)) -> dict:
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден")
