...
SLIDES_DATA = [
    (
        ...
        [
            "Предлагаемая формулировка: «Разработка программного комплекса Garderobus для рекомендаций по выбору одежды с цифровым манекеном на основе данных внешних погодных API»",
            "Фокус на приложении: Garderobus агрегирует прогнозы из API и сервисов партнёров без собственной аппаратуры",
            "Аппаратная часть = интеграции: подключаем готовые метеостанции и датчики партнёров по API",
            "Цифровой манекен собирает луки из вещей пользователя под актуальную погоду и визуализирует образ",
        ],
    ),
    (
