...
def build_document_body() -> str:
    ...
    thesis_points = [
        (
            ...
            "«Разработка программного комплекса Garderobus для рекомендаций по выбору одежды с цифровым манекеном на основе данных внешних погодных API».",
        ),
        (
            "Фокус на приложении.",
            "Основной результат — мобильное и облачное приложение Garderobus: оно агрегирует прогнозы из API и сервисов партнёров и генерирует рекомендации без собственной аппаратуры.",
        ),
        (
            "Аппаратная часть — интеграции.",
            "Используем готовые метеостанции и датчики партнёров как источники данных, подключаясь к ним по API; диплом подчёркивает, что весь «hardware» сводится к программным интеграциям.",
        ),
        (
            "Цифровой манекен.",
            "Приложение строит виртуального манекена и собирает образы из гардероба пользователя под актуальные погодные условия, показывая, как сочетаются его вещи.",
        ),
    ]
